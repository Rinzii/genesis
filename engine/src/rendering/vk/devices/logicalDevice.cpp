...
namespace gen
{
	namespace
	{
		...
		DeviceInfo buildDeviceInfo(vk::PhysicalDevice physicalDevice, const vk::SurfaceKHR & surface)
		{
			...
			for (u32 i = 0; i < memoryProperties.memoryHeapCount; i++)
			{
				...
				{
					totalDeviceMemory += memoryProperties.memoryHeaps[i].size;
				}
			}

			if (surface != nullptr)
			{
				if(doesGpuSupportSurface(physicalDevice, surface) != vk::Result::eSuccess)
				{
					throw VulkanException("GPU does not support surface.");
				}
			}
			...
			if (surface != nullptr)
			{
				presentSupport = physicalDevice.getSurfaceSupportKHR(familyIndex, surface);
			}
			...
			return DeviceInfo{
				...
				.gpu = gpu,
				.totalDeviceLocalMemory = totalDeviceMemory,
		...
		std::vector<DeviceInfo> getAvailableDevices(vk::Instance instance, const vk::SurfaceKHR & surface)
		{
			...
			{
				devices.push_back(buildDeviceInfo(availablePhysicalDevice, surface));
			}
		...
		bool isGpuSuitable(
			const Gpu & gpu, const vk::SurfaceKHR & surface, std::span<const char * const> requiredExtensions, const vk::PhysicalDeviceFeatures & requiredFeatures)
		{
		...
		bool compareGpus(const DeviceInfo & rhs, const DeviceInfo & lhs, const vk::SurfaceKHR surface, const vk::PhysicalDeviceFeatures & requiredFeatures,
				   ...
				   std::optional<u32> selectSpecificGpuIndex)
	{
		...
		{
			if (selectSpecificGpuIndex.value() >= m_gpus.size())
			{
				throw VulkanException("Invalid GPU index specified!");
			}
		...
		{
			m_gpu = buildDeviceInfo(selectBestPhysicalDevice(m_gpus, surface, requiredExtensions, requiredFeatures), surface);
		}
		...
		queueCreateInfos.emplace_back(
			vk::DeviceQueueCreateFlags{}, m_gpu.gpu.graphicsQueueFamilyIndex, 1, &defaultQueuePriority_v);
	...
	vk::PhysicalDevice Device::selectBestPhysicalDevice(std::vector<DeviceInfo> & gpus,
														vk::SurfaceKHR surface,
														std::span<const char * const> requiredExtensions,
														const vk::PhysicalDeviceFeatures & requiredFeatures)
	{
		...
		std::sort(gpus.begin(), gpus.end(), [&](const DeviceInfo & rhs, const DeviceInfo & lhs) {
			return compareGpus(rhs, lhs, surface, requiredFeatures, requiredExtensions);
		});

		if (!isGpuSuitable(gpus.front().gpu, surface, requiredExtensions, requiredFeatures))
		{
			throw VulkanException("No suitable GPU was found!");
		}
		...
		const vk::PhysicalDeviceFeatures & requiredFeatures)
	{
		...
		std::transform(availablePhysicalDevices.begin(),
					   availablePhysicalDevices.end(),
					   devices.begin(),
					   [&](const vk::PhysicalDevice & physicalDevice)
					   {
						   return buildDeviceInfo(physicalDevice, surface);
					   });
