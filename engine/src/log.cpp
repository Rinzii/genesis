...
protected:
	int overflow(int c) override {
		if (c != EOF) {
			...
			char const character = static_cast<char>(c);
...
std::ostream& trace(std::string const& message, std::source_location const& location) {
...
std::ostream& log(std::string const& message, std::source_location const& location) {
	static SingleOutputStream output_stream(std::cout.rdbuf());
	std::lock_guard<std::mutex> lock(log_mutex);
	...
	// Log to console
	output_stream << "[LOG] [" << Time::GetCurrentTime() << "] - " << location.file_name() << ":" << location.line() << " - FUNC: " << location.function_name()
				  << " - " << message << std::endl;
	return output_stream;
}

std::ostream& warn(std::string const& message, std::source_location const& location) {
	static SingleOutputStream output_stream(std::cout.rdbuf());
	std::lock_guard<std::mutex> lock(log_mutex);
	...
	// Log to console
	output_stream << "[WARN] [" << Time::GetCurrentTime() << "] - " << location.file_name() << ":" << location.line() << " - FUNC: " << location.function_name()
				  << " - " << message << std::endl;
	return output_stream;
}

std::ostream& error(std::string const& message, std::source_location const& location) {
...
std::ostream& fatal(std::string const& message, std::source_location const& location) {
	static SingleOutputStream output_stream(std::cerr.rdbuf());
	std::lock_guard<std::mutex> lock(log_mutex);
	...
	// Log to file
	log_stream << "[FATAL] [" << Time::GetCurrentTime() << "] - " << location.file_name() << ":" << location.line() << " - FUNC: " << location.function_name()
			   << " - " << message << std::endl;
